    ...
    unsafe fn acquire_texture(
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if let Some(window) = hosting_window(Retained::into_super(render_layer.clone())) {
                // macOS 26 reports occlusion state with different bit values
                // than NSWindowOcclusionStateVisible, so the bit check always
                // fails there (gfx-rs/wgpu#9430) and nothing ever presents.
                // Use `isVisible` instead: ordered-on-screen windows are
                // acquirable regardless of coverage.
